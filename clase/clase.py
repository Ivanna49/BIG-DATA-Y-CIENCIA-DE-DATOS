...
# Creamos las instrucciones para crear las tablas
...
# Creamos 2 listas con los datos a cargar en las tablas
lista1 = [(1,'Normal 1','Quilmes','Buenos Aires',250),
...
lista2 = [(1,2,1000,'Ramón Mesa',8,1,'rmesa@mail.com'),
          ...
          (7,2,106,'Martín Bossio',0,3,''),
          (8,4,100,'Paula Remmi',3,1,'mail@mail.com'),
          (9,4,1234,'Pedro Gómez',6,2,'')]

print(lista2)

# Ejecuto la carga de varios alumnos y escuelas a la vez
# cur.executemany('INSERT INTO escuelas VALUES (?,?,?,?,?)', lista1)
# cur.executemany('INSERT INTO alumnos VALUES (?,?,?,?,?,?,?)', lista2)

query1 = '''SELECT alumnos.legajo, alumnos.nombre, escuelas.nombre, 
            escuelas.localidad FROM alumnos INNER JOIN escuelas
            ON alumnos.id_escuela = escuelas._id'''

for elemento in cur.execute(query1):
    print(elemento)

# Generar los cambios y cerrar la conexion
con.commit()
con.close()
