...
def borrar():
    resp = messagebox.askquestion('Eliminar', 'Desea eliminar al alumno?')
    if resp == 'yes':
        cur.execute('DELETE FROM alumnos WHERE legajo =' + legajo.get())
        con.commit()
        messagebox.showinfo('Status', 'Alumno eliminado de la bbdd')
        limpiar()
...
def actualizar():
    ...
    con.commit()
    messagebox.showinfo('Status', 'Alumno Actualizado')
    limpiar()
